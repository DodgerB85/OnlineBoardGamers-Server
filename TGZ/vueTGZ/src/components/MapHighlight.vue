...
function changeGhost(event, index, add) {
	...
	if (store.context.action === rf.ACT_BUILD_MON || store.context.action === rf.ACT_FIRST_MON) {
		...
		store.topMenuViews.hubRangesToHighlight = map.getAllOrAnySquaresWithinRangeOfZoneUsingHubs([index], controller.currentPlayerObj().god[0] === rf.ESHU ? 6 : 3, 0)
	}
	// PLACING RESOURCE
		...
		if (store.context.itemBeingAdded !== rf.WATER_TILE) {
			let data = map.getAllCraftsmanDataWithinRangeOfZoneAndOutOfRange([index], controller.currentPlayerObj().god[0] === rf.ESHU ? 6 : 3, rf.RES_TILE_TO_SQ[store.context.itemBeingAdded])
			...
			store.topMenuViews.hubRangesToHighlight.push(map.getAllSquaresWithinRangeOfZone([index], controller.currentPlayerObj().god[0] === rf.ESHU ? 6 : 3, false))
		}
	}
	// PLACING CRAFTSMAN
      ...
      zone.push(index + (map.getSw()*2))
		...
		store.topMenuViews.hubRangesToHighlight.splice(0)
		store.topMenuViews.hubRangesToHighlight = map.getAllOrAnySquaresWithinRangeOfZoneUsingHubs(zone, controller.currentPlayerObj().god[0] === rf.ESHU ? 6 : 3, 0)
	}
}
