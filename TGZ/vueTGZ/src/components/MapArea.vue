...
function clickedMonument(monument) {
	...
	if (!currentMonnument && !model.has_god(controller.currentPlayerObj(), rf.YEMOJA)) return
	...
	store.topMenuViews.hubRangesToHighlight = map.getAllOrAnySquaresWithinRangeOfZoneUsingHubs([monument[0]], model.has_god(controller.currentPlayerObj(), rf.ESHU) ? 6 : 3, 0)
...
function clickedCraftsman(event, craftsmanData, playerIndex) {
	...
	if (store.context.action === rf.ACT_OYA_RUITUALGOOD) {
		...
		store.topMenuViews.hubRangesToHighlight.push(map.getAllSquaresWithinRangeOfZone(map.getCraftsmanZoneFromData(craftsmanData), model.has_god(controller.currentPlayerObj(), rf.ESHU) ? 6 : 3, false))
	...
	if (model.has_god(controller.currentPlayerObj(), rf.AJE_SHALUGA) && store.context.currentRitualGood.length > 0) {
	...
	if (craftsmanCost > 0 && controller.currentPlayerIndex() !== model.getPlayerIndexForCraftsmanPriIndex(craftsmanData[0]) && (/*(model.has_god(controller.currentPlayerObj(), rf.EKWENSU) && model.get_godData(controller.currentPlayerObj(), rf.EKWENSU)[1] > 0) ||*/ (model.has_god(craftsmanPlayer, rf.EKWENSU) && model.get_godData(craftsmanPlayer, rf.EKWENSU)[1] > 0))) {
		...
		if (store.context.EKWENSUdecision === -1) {
			...
			EKWENSUdata.EKWENSUcowsAvailable = model.get_godData(store.players.find((player) => model.has_god(player, rf.EKWENSU)), rf.EKWENSU)[1]
			...
			if (model.has_god(controller.currentPlayerObj(), rf.EKWENSU)) EKWENSUcows = Math.min(model.get_godData(controller.currentPlayerObj(), rf.EKWENSU)[1], desiredEKWENSUcows)
			else if (model.has_god(craftsmanPlayer, rf.EKWENSU)) EKWENSUcows = Math.min(model.get_godData(craftsmanPlayer, rf.EKWENSU)[1], desiredEKWENSUcows)
			...
			if (model.has_god(controller.currentPlayerObj(), rf.EKWENSU)) model.update_godData(controller.currentPlayerObj(), rf.EKWENSU, model.get_godData(controller.currentPlayerObj(), rf.EKWENSU)[1] - EKWENSUcows)
			else if (model.has_god(craftsmanPlayer, rf.EKWENSU)) model.update_godData(craftsmanPlayer, rf.EKWENSU, model.get_godData(craftsmanPlayer, rf.EKWENSU)[1] - EKWENSUcows)
			...
			if (store.context.WATERTOLLpaymentStatus === 1) {
				...
				model.update_godData(store.players[WATERTOLLplayerIndex], rf.WATERTOLL, model.get_godData(store.players[WATERTOLLplayerIndex], rf.WATERTOLL)[1] + rf.WATERTOLLCowToll)
	...
	for (let i = 0; i < store.players[playerIndex].techs.length; i++) {
		if (craftsmanData[1] === rf.BLACKSMITH_TILE) {
			if (store.players[playerIndex].techs[i][0] === rf.BLACKSMITH_TECH) {
				if (model.has_god(controller.currentPlayerObj(), rf.OVIA)) {
					model.update_godData(controller.currentPlayerObj(), rf.OVIA, model.get_godData(controller.currentPlayerObj(), rf.OVIA)[1] + 1)
	...
	for (let i = 0; i < store.players.length; i++) {
		if (model.has_god(store.players[i], rf.QAMATA)) {
			model.update_godData(store.players[i], rf.QAMATA, model.get_godData(store.players[i], rf.QAMATA)[1] + hubsUsed)
			break
		}
	}
...
function isRitualGoodComplete(craftsmanSq) {
	...
	if (!store.context.ignoreAjeShaluga && model.has_god(controller.currentPlayerObj(), rf.AJE_SHALUGA_OLD) && model.getPlayerIndexForCraftsmanPriIndex(store.context.currentRitualGood[0][0]) === controller.currentPlayerIndex()) {
		return true
	}
	return false
}
...
function clickedResource(index) {
	...
	if (isRitualGoodComplete(craftsmanSq)) {
		...
		// Otherwise set up for next ritual good
			...
			store.topMenuViews.hubRangesToHighlight.splice(0)
			store.topMenuViews.hubRangesToHighlight = map.getAllOrAnySquaresWithinRangeOfZoneUsingHubs([store.context.upgradingMonumentProcess[0][0]], model.has_god(controller.currentPlayerObj(), rf.ESHU) ? 6 : 3, 0)
...
function shouldCmanShowInMonProcessBorder(craftsmanData) {
	if (model.has_god(controller.currentPlayerObj(), rf.TSUI_GOAB)) {
