...
export function expandWater(index) {
	...
	//if (getPlayerPrimary_god(controller.currentPlayerObj())[0] === rf.OLOKUN) validWaterSqs = validWaterSqs.concat(rf.CRAFTSMEN_SQS).concat(rf.RESOURCE_SQS)
...
export function getAllSquaresWithinRangeOfZone(zone, range, returnRanges) {
	...
	while (remainingRange > 0) {
		...
		let validWaterSqs = [rf.WATER_SQ]
		//if (model.getPlayerPrimary_god(controller.currentPlayerObj())[0] === rf.OLOKUN) validWaterSqs = validWaterSqs.concat(rf.CRAFTSMEN_SQS).concat(rf.RESOURCE_SQS)
		let waterSqs = []
		...
		// Expand for Simbi
		if (model.has_god(controller.currentPlayerObj(), rf.SIMBI)) {
			let simbiSqs = []
			for (let j = 0; j < thisRange.length; j++) {
...
export function getAllUndepletedResourceSquaresToHighlight(craftsmanData, range, excludeArr) {
	...
	if (model.has_god(controller.currentPlayerObj(), rf.AGWU_NSI)) {
	...
	for (let i = allRes.length - 1; i >= 0; i--) {
		if (!model.has_god(controller.currentPlayerObj(), rf.ATETE) && store.depletedResources.includes(allRes[i])) allRes.splice(i, 1)
		else if (model.has_god(controller.currentPlayerObj(), rf.ATETE) && duplicateElements.includes(allRes[i])) allRes.splice(i, 1)
	}
	return allRes
}
