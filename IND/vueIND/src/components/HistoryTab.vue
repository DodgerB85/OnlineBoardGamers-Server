...
<template>
	<transition name="fade">
		<div
			id="history"
			v-if="store.topMenuViews.showHistory"
			:style="{
				position: store.historyOnly ? 'inline-block' : 'absolute',
				paddingTop: '5px',
				left: '2px',
				top: store.historyOnly ? '0px' : '60px',
				width: store.historyOnly ? '1000px' : '450px',
			}">
			<!-- TOGGLE DIV-->
			...
			<div id="historyMainDiv" class="reverseHistory">
				<template v-for="(entry, idx) in store.computedHistory" :key="idx">
					<HistoryEntry :entry="entry" :entry_-i-d="idx" />
				</template>
			</div>
