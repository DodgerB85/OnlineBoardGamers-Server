  ...
  try {
    ...
    if (data.bugEntrySuccess)
      ...
      store.topMenuViews.bugErrorText =
        'Sorry, there was a problem.<br/>Please email the webmaster directly or report on the Discord'
  } catch (error) {
...
export async function saveGame(saveRewind, saveContext) {
  ...
  if (store.gameflow.turnOrder.length > 0)
    postData.nextPlayer =
      store.players[store.gameflow.turnOrder[0]].name // USED > goes to currentPlayers
  else {
  ...
  if (store.gameflow.phase === rf.PHASE_GAME_OVER) {
    ...
    for (let i = 0; i < finalRes.length; i++)
      finalPositionsNames.push(store.players[finalRes[i][0]].name)
  ...
  try {
    ...
    // Broadcast update
    if (WS.CNSwebSocket && WS.CNSwebSocket.readyState === 1)
      WS.CNSwebSocket.send('NEWDATATS' + String(personal.gameID) + String(personal.latestUpdate))
    else if (WS.CNSwebSocket && personal.liveWS) {
      await WS.StartWebSocket()
      await funcs.sleep(2000)
      if (personal.liveWS && WS.CNSwebSocket.readyState === 1)
        WS.CNSwebSocket.send('NEWDATATS' + String(personal.gameID) + String(personal.latestUpdate))
...
async function updateDataFromLoadRewind() {
  ...
  try {
    ...
    personal.latestUpdate = data.latestUpdate
    personal.secondsToNextKickout = data.secondsToNextKickout
    if (WS.CNSwebSocket.readyState === 1)
      WS.CNSwebSocket.send('NEWDATATS' + String(personal.gameID) + String(personal.latestUpdate))
