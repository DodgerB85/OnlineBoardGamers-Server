	...
	store.hexes.forEach((t) => {
		store.hexes.forEach((t2) => {
			...
			directionVectors.forEach((dv) => {
				if (t.hex.q === t2.hex.q + dv[0] && t.hex.r === t2.hex.r + dv[1] && t.hex.s === t2.hex.s + dv[2]) {
...
export function setPlaceableTiles() {
	...
	store.hexes.forEach((t) => {
		...
		directionVectors.forEach((dv) => {
			...
			if (!neighbour && !store.context.placeableTiles.find((h) => h.q == newHex.q && h.r == newHex.r && h.s == newHex.s)) store.context.placeableTiles.push({ ...newHex, isPendingPlacement: false })
		})
	})
...
const getOldBoysConnection = (links) => {
	...
	for (const link of links) {
		const oldBoys = store.oldBoysNetwork.filter((ob) => ob.map((c) => c.id).includes(link[0].id) || ob.map((c) => c.id).includes(link[1].id))

		for (const ob of oldBoys) {
			if (!foundOldBoys.find((o) => [o[0].id, o[1].id].includes(ob[0].id) && [o[0].id, o[1].id].includes(ob[1].id))) {
...
const scanConnections = (links) => {
	...
	links.forEach((link) => {
		if (rf.HEX_PARTY_ROTATABLE.includes(link[0].hexRef) || rf.HEX_PARTY_ROTATABLE.includes(link[1].hexRef)) result.hasParty = true
		if (rf.CANNES_HEXES.includes(link[0].hexRef) || rf.CANNES_HEXES.includes(link[1].hexRef)) result.hasCannes = true
		if (rf.HEX_PHONE_TILES.includes(link[0].hexRef) || rf.HEX_PHONE_TILES.includes(link[1].hexRef)) result.hasPhone = true
	})
...
const getOldBoysInPlayerNetwork = (player) => {
	...
	const scanOldBoys = (links = [], store = []) => {
		...
		for (const oldBoyLink of connectedOldBoys) {
			if (!store.find((r) => [r[0].id, r[1].id].includes(oldBoyLink[0].id) && [r[0].id, r[1].id].includes(oldBoyLink[1].id))) {
	...
	for (const party of getPartiesInNetwork(tempNetwork)) {
		...
		for (const entrance of entrances) {
			for (const oldBoy of store.oldBoysNetwork) {
				if (linkIncludesHex(oldBoy, entrance) && !tempNetwork.find((tl) => linkIncludesHex(tl, entrance))) {
					const ob = []
					scanOldBoys([oldBoy], ob)
					ob.forEach((o) => {
						tempNetwork.push(o)
...
export function setPlaceableLinks(player, cigars) {
        ...
        if (newLinkExists) {
          return
				...
				const linkPlaced = allLinks.find((linkInAllLinks) => {
					const h1 = store.hexes.find((h) => h.hex.q == linkInAllLinks[0].hex.q && h.hex.r === linkInAllLinks[0].hex.r)
					const h2 = store.hexes.find((h) => h.hex.q == linkInAllLinks[1].hex.q && h.hex.r === linkInAllLinks[1].hex.r)
					if (!h1 && !h2) return
	...
	if (playerLinks.length <= 4 || cigars) {
		...
		playerLinks.map((l1) => {
			...
			n1.forEach((l) => {
				...
				if (
					!allLinks.find((al) => {
						return (al[0].id === l1[0].id && al[1].id === nl.id) || (al[0].id === nl.id && al[1].id === l1[0].id)
					})
				) {
			...
			n2.forEach((l) => {
				...
				if (
					!allLinks.find((al) => {
						return (al[0].id === l1[1].id && al[1].id === nl.id) || (al[0].id === nl.id && al[1].id === l1[1].id)
					})
				) {
			...
			rf.CANNES_HEXES.forEach((ch) => {
				...
				const findCN = (hex) => {
					...
					hexNeighbours.forEach((hn) => {
						if (
							!allLinks.find((al) => {
								return (al[0].id === hn && al[1].id === hex.id) || (al[0].id === hex.id && al[1].id === hn)
							})
						) {
		...
		if (hasPhone) {
			rf.HEX_PHONE_TILES.forEach((ch) => {
				...
				phoneNeighbours.forEach((hn) => {
					if (
						!allLinks.find((al) => {
							return (al[0].id === hn.id && al[1].id === hex.id) || (al[0].id === hex.id && al[1].id === hn.id)
						})
					) {
							...
							pz.forEach((x) => {
								if (rf.HEX_PARTY_ROTATABLE.includes(x.hexRef) && h1.id != x.id && h2.id != x.id) {
									store.context.neighbours[x.id].forEach((n) => {
										if (
											!allLinks.find((al) => {
												return (al[0].id === x.id && al[1].id === n) || (al[0].id === n && al[1].id === x.id)
											})
										) {
	...
	newLinks = newLinks
		...
		.filter((subArray, index, self) => {
			const [A, B] = subArray
			const duplicateIndex = self.findIndex((arr, i) => i > index && JSON.stringify(arr[0]) === JSON.stringify(A) && JSON.stringify(arr[1]) === JSON.stringify(B))
...
export function getLinkSVGpoints(link, cigar, forHighlight) {
	...
	else if (Math.abs(Math.round(p.x) - Math.round(q.x)) - Math.round((store.refSize / 8 / store.canvasSize) * 600) < 2 && Math.round(p.y) > Math.round(q.y)) {
	...
	else if (Math.abs(Math.round(p.x) - Math.round(q.x)) - Math.round((store.refSize / 8 / store.canvasSize) * 600) < 2) {
		// Move away form centre
		if (!cigar) {
			p.x -= ((50 * 0.33 * store.refSize) / 2200 / store.canvasSize) * 447
			p.y += ((50 * 0.66 * store.refSize) / 2200 / store.canvasSize) * 447
...
export function getHexPoints(forHighlight) {
	...
	const absolutePoints = pointyPoints.map(([x, y]) => `${((x * hexSideLength) / canvasSize) * 100},${((y * hexSideLength) / canvasSize) * 100}`)
...
export function setRemovableLinks(player) {
	...
	const filterHangingOldBoys = (oldBoys) => {
		...
		const scan = (oldBoy) => {
			...
			if (playerLinks.find((pl) => linkIncludesHex(pl, h1)) && playerLinks.find((pl) => linkIncludesHex(pl, h2))) {
				return true
			}
			...
			const scanObNetwork = (oldBoy) => {
				...
				const nextLink = rest.filter((x) => ((rf.HEX_PARTY_ROTATABLE.includes(h1.hexRef) || rf.HEX_PARTY_ROTATABLE.includes(h2.hexRef)) && (rf.HEX_PARTY_ROTATABLE.includes(x[0].hexRef) || rf.HEX_PARTY_ROTATABLE.includes(x[1].hexRef)) && store.context.partyZones.find((pz) => pz.find((p) => p.id == h1.id || p.id == h2.id) && pz.find((p) => p.id == x[0].id || p.id == x[1].id))) || x[0].id == h1.id || x[1].id == h2.id || x[0].id == h2.id || x[1].id == h1.id || ((rf.CANNES_HEXES.includes(h1.hexRef) || rf.CANNES_HEXES.includes(h2.hexRef)) && (rf.CANNES_HEXES.includes(x[0].hexRef) || rf.CANNES_HEXES.includes(x[1].hexRef))) || ((rf.HEX_PHONE_TILES.includes(h1.hexRef) || rf.HEX_PHONE_TILES.includes(h2.hexRef)) && (rf.HEX_PHONE_TILES.includes(x[0].hexRef) || rf.HEX_PHONE_TILES.includes(x[1].hexRef))))
			...
			const connectedLinks = playerLinks.reduce((prev, curr) => {
				...
				if (hasParty && parties.length > 0) {
					connectedPartyEntrances = parties[0].filter((pt) => rf.HEX_PARTY_ROTATABLE.includes(pt.hexRef))
				}
          ...
          if (
            ...
            return
					...
					if ((hasParty && connectedPartyEntrances.map((pe) => pe.hexRef).includes(ob[0].hexRef)) || connectedPartyEntrances.map((pe) => pe.hexRef).includes(ob[1].hexRef)) {
						prev.push(curr)
						return
					}

					if ((hasCannes && rf.CANNES_HEXES.includes(ob[1].hexRef)) || (hasCannes && rf.CANNES_HEXES.includes(ob[0].hexRef))) {
						prev.push(curr)
						return
					}
				})
	...
	playerLinks.forEach((l) => {
		...
		function checkConnection(link) {
			...
			const h1 = link[0]
			const h2 = link[1]
			const hexLinks = rest.filter((x) => ((rf.HEX_PARTY_ROTATABLE.includes(h1.hexRef) || rf.HEX_PARTY_ROTATABLE.includes(h2.hexRef)) && (rf.HEX_PARTY_ROTATABLE.includes(x[0].hexRef) || rf.HEX_PARTY_ROTATABLE.includes(x[1].hexRef)) && store.context.partyZones.find((pz) => pz.find((p) => p.id == h1.id || p.id == h2.id) && pz.find((p) => p.id == x[0].id || p.id == x[1].id))) || x[0].id == h1.id || x[1].id == h2.id || x[0].id == h2.id || x[1].id == h1.id || ((rf.CANNES_HEXES.includes(h1.hexRef) || rf.CANNES_HEXES.includes(h2.hexRef)) && (rf.CANNES_HEXES.includes(x[0].hexRef) || rf.CANNES_HEXES.includes(x[1].hexRef))) || ((rf.HEX_PHONE_TILES.includes(h1.hexRef) || rf.HEX_PHONE_TILES.includes(h2.hexRef)) && (rf.HEX_PHONE_TILES.includes(x[0].hexRef) || rf.HEX_PHONE_TILES.includes(x[1].hexRef))))
			hexLinks.forEach((hl) => {
