...
function getViewbox() {
	...
	let vertShift = (((((netGridNumRows * 0.866) / -2) * 260) / store.canvasSize) * store.refSize) / 2400
...
function localAddHexToMap(tile, hexRef, rotation) {
	...
	if (placementError) {
		...
		model.addHexToMap(tile, hexRef)

		// 1. Tag the newest hex
		const newHex = store.hexes[store.hexes.length - 1]
		newHex.isNew = true
...
<template>
	<div
		...
		<svg id="hexSVG" v-if="store.hexes" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.com/svgjs" :viewBox="getViewbox()">
			...
			<defs>
				...
				<g v-if="store.tableJunk.length > 0">
					<!-- Junk Hex Imgs -->
					<pattern v-for="(num, index) in [1, 2, 3, 4, 5, 6, 7]" :key="index" :id="'patternJunk' + num" height="100%" width="100%" patternContentUnits="objectBoundingBox">
						<image height="1" width="1" preserveAspectRatio="none" :xlink:href="view.getImage('hexJunk' + String(num))" />
					</pattern>
				</g>
				<!-- Cigar -->
			...
			<!-- Add table Top Edge -->
			<rect v-if="store.tableUp !== 0" :x="(((-store.tableLeft * store.refSize) / 1 / 3) * 260) / store.canvasSize + 10" :y="(((-store.tableUp * rf.hexSmallRatio * store.refSize * 1) / 3) * 260) / store.canvasSize - (((0.5 * store.refSize) / 8) * 3 * 262.7) / store.canvasSize + 10 - store.tableBulk" :width="((((store.tableLeft + store.tableRight) * store.refSize) / 1 / 3) * 262.7) / store.canvasSize - 20" :height="store.tableBulk - 20" fill="brown" stroke="black" stroke-width="20" />

			<!-- Add table Botton Edge -->
			<rect v-if="store.tableUp !== 0" :x="(((-store.tableLeft * store.refSize) / 1 / 3) * 260) / store.canvasSize + 10" :y="(((store.tableDown * rf.hexSmallRatio * store.refSize * 1) / 3) * 260) / store.canvasSize + (((0.5 * store.refSize) / 8) * 3 * 262.7) / store.canvasSize + 10" :width="((((store.tableLeft + store.tableRight) * store.refSize) / 1 / 3) * 262.7) / store.canvasSize - 20" :height="store.tableBulk - 20" fill="brown" stroke="black" stroke-width="20" />

			<!-- Add table Left Edge -->
			<rect v-if="store.tableUp !== 0" :x="(((-store.tableLeft * store.refSize) / 1 / 3) * 260) / store.canvasSize + 10 - store.tableBulk" :y="(((-store.tableUp * rf.hexSmallRatio * store.refSize * 1) / 3) * 262.7) / store.canvasSize - (((0.5 * store.refSize) / 8) * 3 * 262.7) / store.canvasSize + 10 - store.tableBulk" :height="((((store.tableUp + store.tableDown) * rf.hexSmallRatio * store.refSize * 1) / 3) * 262.7) / store.canvasSize + ((store.refSize / 8) * 3 * 262.7) / store.canvasSize - 20 + store.tableBulk * 2" :width="store.tableBulk - 20" fill="brown" stroke="black" stroke-width="20" />

			<!-- Add table Right Edge -->
			<rect v-if="store.tableUp !== 0" :x="(((store.tableRight * store.refSize) / 1 / 3) * 260) / store.canvasSize + 10" :y="(((-store.tableUp * rf.hexSmallRatio * store.refSize * 1) / 3) * 262.7) / store.canvasSize - (((0.5 * store.refSize) / 8) * 3 * 262.7) / store.canvasSize + 10 - store.tableBulk" :height="((((store.tableUp + store.tableDown) * rf.hexSmallRatio * store.refSize * 1) / 3) * 262.7) / store.canvasSize + ((store.refSize / 8) * 3 * 262.7) / store.canvasSize - 20 + store.tableBulk * 2" :width="store.tableBulk - 20" fill="brown" stroke="black" stroke-width="20" />

			<text x="0" y="0" text-anchor="middle" dominant-baseline="central" fill="white" font-size="16">Table</text>

			<!-- Add Hexes -->
			<g v-for="tile in store.hexes" :key="tile.id" :transform="'rotate(' + tile.rotation * 60 + ' ' + hexCenter(tile.hex, false, true) + ')' + hexCenter(tile.hex, false)">
				<polygon :id="'hex' + tile.id" @click="hexClicked(tile)" :points="map.getHexPoints(false)" class="mapHexSVG" :class="{ 'hex-plop': tile.isNew }" :fill="`url(#pattern${tile.hexRef})`" />
				<ellipse v-if="tile.isNew" class="tile-ripple" :cx="hexCenter(tile.hex, true, false)[0] * 0" :cy="hexCenter(tile.hex, true, false)[1] * 0" :rx="store.refSize / store.canvasSize * 60" :ry="store.refSize / store.canvasSize * 60" />
			</g>

			<!-- Add Junk Hex -->
			<polygon v-for="(junkData, idx) in store.tableJunk" :key="idx" :points="map.getHexPoints(false)" :transform="hexCenter(junkData[0], false)" :fill="`url(#patternJunk${junkData[1]})`" class="mapJunkSVG"></polygon>
			...
			<g v-if="store.context.action === rf.ACT_PLACE_HEX">
				<polygon
					...
					:points="map.getHexPoints(false)"
					:transform="'rotate(' + store.context.hexBeingAddedRotation * 60 + ' ' + hexCenter(tile, false, true) + ')' + hexCenter(tile, false)"
			...
			<g v-if="store.context.action === rf.ACT_MOVE_PIRATE">
				<g v-for="(partyZone, partyNum) in store.context.partyPointsToHighlight" :key="partyNum">
					<polygon v-for="(hexPoints, idx) in partyZone" class="partyZoneForPirate" :key="idx" @click="addPirateToParty(partyNum)" :points="hexPoints" @mouseover="store.context.partyMouseOver = partyNum" @mouseout="store.context.partyMouseOver = -1" :fill="getPartyOptionFill(partyNum)"></polygon>
				</g>
			</g>
			...
			<g v-for="(player, idx) in store.players" :key="idx">
				<polygon
					:key="link.id"
					v-for="link in player.links"
					:points="map.getLinkSVGpoints(link, false)"
					@click="linkClicked(player, link)"
					@mouseover="link.isMouseOver = true"
					@mouseout="link.isMouseOver = false"
					:class="{ 'link-fade-in': link.isNew }"
					:style="{
			...
			<g v-if="store.context.action === rf.ACT_ADD_LINK || store.context.action === rf.ACT_READD_LINK">
				<polygon
					:key="link.id"
					v-for="link in store.context.placeableLinks"
					:points="map.getLinkSVGpoints(link, false)"
					@click="model.addLink(controller.currentPlayerObj(), link)"
					@mouseover="link.isMouseOver = true"
					@mouseout="link.isMouseOver = false"
					class="newLink"
					:style="{
			...
			<!-- Add Cigars -->
			<polygon :key="link.id" v-for="link in store.oldBoysNetwork" class="boardCigarPolygon" :class="{ 'link-fade-in': link.isNew }" :points="map.getLinkSVGpoints(link, true)" :fill="'url(' + getCigarPattern(link) + ')'"></polygon>
			...
			<g v-if="store.context.action === rf.ACT_ADD_CIGAR">
				<polygon
					:key="link.id"
					v-for="link in store.context.placeableLinks"
					:points="map.getLinkSVGpoints(link, true, true)"
					@click="model.addCigar(link)"
					@mouseover="link.isMouseOver = true"
					@mouseout="link.isMouseOver = false"
					:fill="'url(' + getCigarPattern(link) + ')'"
					class="newCigar"
					:style="{
						'stroke-width': store.refSize / 240 + 'px',
					}"></polygon>
		...
		<template v-if="store.debugVars.showHexImgs">
			<div
				v-for="(tile, index) in store.hexes"
				:key="index"
				class="hexTileDiv"
				:style="{
				...
				<img class="hexIMG" :src="view.getImage('hex' + String(rf.HEX_PROD_MOVIE_SCIFI))" :alt="rf.HEX_PROD_MOVIE_SCIFI" />
			</div>
		</template>
