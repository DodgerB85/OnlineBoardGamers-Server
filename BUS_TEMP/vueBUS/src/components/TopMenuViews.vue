...
async function submitBug() {
	...
	var csrftoken = refFuncs.getCookie("csrftoken")
...
function loadRewind() {
	...
	setTimeout(function () {
		model.topMenuViews.showRewindPanel = false
		IO.loadRewind()
	}, 500)
}
...
function getStatsExcludeVotes(returnPlayers = false) {
	...
	for (const player in model.statsExcludeVotesData) {
		// Use const or let for player
		if (model.statsExcludeVotesData[player] === true) {
			votes += 1
...
function getDeleteVotes(returnPlayers = false) {
	...
	for (const player in model.deleteVotesData) {
		// Use const or let for player
		if (model.deleteVotesData[player] === true) {
			votes += 1
			if (players === "None") players = String(player)
			else players += ", " + player
		}
...
<template>
	<!-- BUG REPORT -->
	<div id="bugReport" v-if="model.topMenuViews.showBug">
		<h2>Bug Report</h2>
		<p>
	...
	<!-- NOTES -->
	<div id="notesBox" v-if="model.topMenuViews.showNotes">
	...
	<transition name="fade">
		<div id="wholeChat" v-if="model.topMenuViews.showChat">
			<div id="chatBox">
			...
			<div id="messageList">
				<div class="chatentry" v-for="(message, index) in model.chatData" v-bind:key="index">
					<div class="header">
	...
	<!-- REWIND PANEL -->
	<transition name="fade">
		<div id="rewindPanel" v-if="model.topMenuViews.showRewindPanel" :style="{
			...
			<hr />
			<div
				v-if="model.gameflow.phase !== constants.PHASE_GAME_OVER && !personal.trainingGame && personal.pov != undefined && personal.pov >= 0">
				...
				<button v-if="!personal.votedToExclude" class="actionsLineButton"
					@click="localCastVote(constants.STATS_EXCLUDE_VOTE_TOPIC)">Vote to
					Exclude
					Game from Stats</button>
			</div>
			<div
				v-if="model.gameflow.phase !== constants.PHASE_GAME_OVER && !personal.trainingGame && personal.pov >= 0">
				...
				<button v-if="!personal.votedToDelete" class="actionsLineButton"
					@click="localCastVote(constants.DELETE_VOTE_TOPIC)">Vote
					to
					Delete
					Game</button>
			</div>
	...
	<!-- HISTORY -->
	<transition name="fade">
		<div id="history" v-if="model.topMenuViews.showHistory">
			<div id="historyToggleDiv">
				...
				<br />
				<span v-if="model.topMenuViews.showReplay">Click entry to jump to that point in the replay</span>
				<span v-else>Click Lines / Buildings / VRROOOMM to highlight board</span>
				<!--<br /><button @click="toggleReplayMode()"><span v-if="model.topMenuViews.showReplay">Exit</span><span
            v-else>Enter</span> Replay Mode</button>-->
			</div>
			<div id="historyMainDiv" class="reverseHistory">
				<template v-for="(entry, index1) in model.history" v-bind:key="index1">
