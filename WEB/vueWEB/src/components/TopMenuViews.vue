...
<template>
  ...
  <transition name="slideRes">
    <div id="reserve" v-if="store.viewSettings.showReserve">
      <!-- DRAW PILE -->
      <h2>Unused Tiles - including opponents' stored tiles and draw piles (where you haven't seen the number of actions)
      </h2>
        ...
        <template v-for="(tileID, idx) in getReserveTileIDs()" :key="idx">
          <div class="tileAndActionsNumberDiv">
            ...
            <div class="singleTileDiv">
              <svg class="singleTileSVG" xmlns="http://www.w3.org/2000/svg"
                :viewBox="rf.ALL_RECT_TILES.includes(tileID) ? '0 0 50 100' : '0 0 100 100'">
                <polygon class="singleTilePolygon" x="0" y="0"
                  :points="view.getPolygonPointsFromTileID(tileID, 0, 0, 0, 50)"
                  :fill="view.getTilePatternFromID(tileID)" />
              </svg>
            </div>
          </div>
        </template>
