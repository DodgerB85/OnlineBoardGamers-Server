...
from Lobby.sharedFunctions.sharedNotifications import SN_sendInviteNotifications, SN_M_T_sendTournamentGameStartNotification
...
def buildFCMstartingOptions(post_data):
    ...
    option_names = [
        "short", "noMilestones", "noCeoMilestone", "noRadioMilestone", "hardChoices",
        "fryChefs", "kimchi", "sushi", "noodles", "gourmet", "movieStars",
        "massMarketers", "nightShift", "ruralMarketers", "newDistricts",
        "newDistrictsApp", "newDistrictsPark", "newDistrictsAll", "coffee",
        "ketchupMilestone", "newMilestones", "lobbyists", "reservePrice",
        "strictPaydayFridge", "sandboxMode", "learningGame", "experiencedGame",
        "urbanPlanning", "urbanPlanningPlus", "jazzMusicians", "dumplings",
        "deliveryDrivers", "hawkers"
    ]
    options.extend(str(post_data[opt]) for opt in option_names if opt in post_data)
    return ",".join(options) if options else ""

@login_required()
def create_fcm_game(request, is_tournament=False, is_mini_tournament=False, tournament=None, round_number_string=None, current_players_usernames=None):
    """
    ...
    if is_tournament or is_mini_tournament:
        if not tournament or not round_number_string:
            raise ValueError("Tournament and round_number_string required for tournament games")
        ...
        if is_mini_tournament:
            ...
            max_players = len(current_players_usernames) if current_players_usernames else tournament.maxGamePlayers
        game_status = "ACTIVE"     
    ...
    # Otherwise, create a normal game
        ...
        usernames = [request.POST.get(f"player{i}") for i in range(2, 7) if request.POST.get(f"player{i}")]
        ...
        game_name = f"[{request.POST['scenario']}] {request.POST['gameName']}" if "scenario" in request.POST else request.POST["gameName"]
        ...
        starting_options = buildFCMstartingOptions(request.POST)  # Use the extracted function
        # Now exclude stats if any china expansion is in starting options
        # Split the string into a list
        options_for_SE = starting_options.split(",") if starting_options != "" else [] 
        if any(x in options_for_SE for x in ["42", "43", "44", "45"]):
        ...
        rewind_consent = "2" * max_players if "allowRewind" in request.POST else ""#"0" * max_players
        ...
        if "fcmAI" in request.POST:
            ...
            invited_players = [get_object_or_404(User, username=username) for username in usernames]
            if invited_players:
                game_status = "WAITING"
    ...
    if is_tournament or is_mini_tournament:
        for username in usernames_to_notify:
            ...
            SN_M_T_sendTournamentGameStartNotification(
                request, "FCM", username, new_game.maxPlayers, new_game.gameName,
                new_game.currentTurnString(), getattr(new_game, "id"), False, tournamentType
            )
        return getattr(new_game, "id")
    
    # Now handle normal games
    if usernames_to_notify:
        SN_sendInviteNotifications(request, usernames_to_notify, new_game.getGameName(), max_players, "FCM")
    if "trainingGame" in request.POST:
        ...
        messages.success(request, SF_getGameCreationJsonReturn("FCM", getattr(new_game, "id")))
...
#        HttpResponseRedirect for normal games, or game ID for tournament games.
#    """
#    if not is_tournament and request.method != "POST":
#        return JsonResponse({"error": "POST request required."}, status=400)
#
#    # Initialize game parameters
#    game_name = ""
...
#    invited_players = []
#
#    def build_starting_options(post_data):
#        """Helper to build starting options efficiently."""
#        options = []
#        if "trainingGame" in post_data:
#            options.extend(["101", post_data["trainingGame"]])
#        if "fcmAI" in post_data:
...
#
#        seat_offset = randint(0, max_players - 1)
#        starting_options = build_starting_options(request.POST)
#        game_pace = request.POST["pace"]
#        creator = request.user
#        host = request.user
#        kickout_duration = request.POST["kickoutDuration"]
...
#            new_game.invitedPlayers.add(player)
#
#        if not is_tournament and "trainingGame" in request.POST:
#            new_game.startGame(request)
#
#        new_game.save()
#
...
#            SN_M_T_sendTournamentGameStartNotification(
#                request, "FCM", username, new_game.maxPlayers, new_game.gameName,
#                new_game.currentTurnString(), getattr(new_game, "id"), False
#            )
#        return getattr(new_game, "id")
#    else:
#        if usernames_to_notify:
