...
export default defineConfig(({ command }) => ({
  ...
  server: {
   ...
   base: command === 'serve' ? '/static/' : 'https://www.onlineboardgamers.com/static/KFW',
  build: {
    outDir: resolve('../static/KFW/KFWvuedist'),
    assetsDir: './assets',
    manifest: false,
    emptyOutDir: true,
    target: 'es2015',
      ...
      output: {
        ...
        assetFileNames: (assetInfo) => {
          ...
          if (/\.(jpg|png|jpe?g|gif|svg|webp|webm|mp3)$/.test(assetInfo.name)) {
            if (assetInfo.originalFileNames[0].startsWith('../../static/KFW/images/tiles/')) {
              return `images/tiles/[name].${extType}`;
            } else if (assetInfo.originalFileNames[0].startsWith('../../static/KFW/images/contracts/')) {
              return `images/contracts/[name].${extType}`;
            }else if (assetInfo.originalFileNames[0].startsWith('../../static/KFW/images/extensions/')) {
              return `images/extensions/[name].${extType}`;
            } else if (assetInfo.originalFileNames[0].startsWith('../../static/KFW/images/')) {
