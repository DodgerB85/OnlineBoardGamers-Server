...
def migrate_aqy_games(apps, schema_editor):
    ...
    for aqy_game in aqy_games:
        ...
        new_game = Game.objects.create(
            ...
            deleteGameVotes=aqy_game.deleteGameVotes
            if hasattr(aqy_game, "deleteGameVotes")
            else None,
            activeVotes=aqy_game.activeVotes
            if hasattr(aqy_game, "activeVotes")
            else None,
            # Copy AQY-specific fields
            playerTradeData=aqy_game.playerTradeData
            if hasattr(aqy_game, "playerTradeData")
            else "",
            # Copy foreign keys
            creator=aqy_game.creator,
            host=aqy_game.host,
            relatedMainTournament=aqy_game.relatedMainTournament
            if hasattr(aqy_game, "relatedMainTournament")
            else None,
            relatedMiniTournament=aqy_game.relatedMiniTournament
            if hasattr(aqy_game, "relatedMiniTournament")
            else None,
        )
        ...
        for idx, player in enumerate(player_list):
            ...
            if idx == 0:
                current_move_time = (
                    aqy_game.player0currentMoveTime
                    if hasattr(aqy_game, "player0currentMoveTime")
                    else ""
                )
                current_move_data = (
                    aqy_game.player0currentMoveData
                    if hasattr(aqy_game, "player0currentMoveData")
                    else ""
                )
            elif idx == 1:
                current_move_time = (
                    aqy_game.player1currentMoveTime
                    if hasattr(aqy_game, "player1currentMoveTime")
                    else ""
                )
                current_move_data = (
                    aqy_game.player1currentMoveData
                    if hasattr(aqy_game, "player1currentMoveData")
                    else ""
                )
            elif idx == 2:
                current_move_time = (
                    aqy_game.player2currentMoveTime
                    if hasattr(aqy_game, "player2currentMoveTime")
                    else ""
                )
                current_move_data = (
                    aqy_game.player2currentMoveData
                    if hasattr(aqy_game, "player2currentMoveData")
                    else ""
                )
            elif idx == 3:
                current_move_time = (
                    aqy_game.player3currentMoveTime
                    if hasattr(aqy_game, "player3currentMoveTime")
                    else ""
                )
                current_move_data = (
                    aqy_game.player3currentMoveData
                    if hasattr(aqy_game, "player3currentMoveData")
                    else ""
                )
