...
def migrate_tgz_games(apps, schema_editor):
    ...
    for tgz_game in tgz_games:
        ...
        new_game = Game.objects.create(
            ...
            statsExcludeConsent=tgz_game.statsExcludeConsent
            if hasattr(tgz_game, "statsExcludeConsent")
            else None,
            deleteGameVotes=tgz_game.deleteGameVotes
            if hasattr(tgz_game, "deleteGameVotes")
            else None,
            activeVotes=tgz_game.activeVotes
            if hasattr(tgz_game, "activeVotes")
            else None,
            # Copy foreign keys
            creator=tgz_game.creator,
            host=tgz_game.host,
            relatedMainTournament=tgz_game.relatedMainTournament
            if hasattr(tgz_game, "relatedMainTournament")
            else None,
            relatedMiniTournament=tgz_game.relatedMiniTournament
            if hasattr(tgz_game, "relatedMiniTournament")
            else None,
        )
