...
#    path('sitemap.xml', sitemap, {'sitemaps': sitemaps}, name='sitemap'),
    ...
    path('reset/<uidb64>/<token>/', auth_views.PasswordResetConfirmView.as_view(template_name="Lobby/password/password_reset_confirm.html", form_class=SetPasswordFormCustom), name='password_reset_confirm'),
    
    path('reset/done/', auth_views.PasswordResetCompleteView.as_view(template_name='Lobby/password/password_reset_complete.html'), name='password_reset_complete'),      
    # Change Password
    path('change-password/', auth_views.PasswordChangeView.as_view(template_name='Lobby/password/password_change.html', success_url = '/'), name='change_password'),
    
    path('i18n/', include('django.conf.urls.i18n')),
    path('favicon.ico', RedirectView.as_view(url=settings.STATIC_URL + 'Lobby/favicon.ico')),
]
